...
# create nginx user/group first, to be consistent throughout docker variants
    && addgroup -g 700 -S nginx \
    && adduser -S -D -H -u 700 -h /var/cache/nginx -s /sbin/nologin -G nginx -g nginx nginx \
...
# create a docker-entrypoint.d directory
    && mkdir -p /docker-entrypoint.d/www \
    && mkdir -p /docker-entrypoint.d/conf.d && \
  ...
  mkdir -p /app && \
  ln -s /var/www /app && \
  ln -s /etc/nginx/conf.d /app && \
  rm -rf /etc/nginx/conf.d/default.conf && \
  rm -rf /var/cache/apk/*

COPY shell/docker-entrypoint.sh /docker-entrypoint.d
COPY www/index.php /docker-entrypoint.d/www/
COPY config/default.conf /docker-entrypoint.d/conf.d/
COPY shell/start.sh /docker-entrypoint.d
COPY shell/10-listen-on-ipv6-by-default.sh /docker-entrypoint.d
COPY shell/20-envsubst-on-templates.sh /docker-entrypoint.d
COPY shell/30-tune-worker-processes.sh /docker-entrypoint.d
WORKDIR /app
VOLUME ["/app"]
EXPOSE 80
STOPSIGNAL SIGQUIT
